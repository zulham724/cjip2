...
@section('content')
    <div class="col-8 col-m-8">
        @foreach($pertanian as $pert)
            <div class="card post__card">
                <div class="post__header">
                    <div class="believe">
                        <img alt="{{$pert->userId->name}}" class="believe__avatar" style="float: left" src="{{Voyager::image($pert->userId->avatar)}}">
                    </div>
                    <h4 class="post__title">{{$pert->nama_wisata}}</h4>
                    <p class="post__category">{{$pert->userId->name}}</p>
                </div>
                ...
                <article class="post__content">
                    <div class="carousel">
                        <div class="slider carousel__slider--images">
                            @php
                                $images = json_decode($pert->potensiId->foto)
                            @endphp
                            @foreach($images as $image)
                                <div class="carousel__slide">
                                    <img src="{{Voyager::image($image)}}" style="max-width: 758px;" alt="">
                                    <p class="carousel__caption">{{$pert->lokasi}}</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    <p>{!! str_limit($pert->keterangan, 300, '....')!!} <a href='{{route('pertanian.detail', $pert->id)}}' class='btn btn-primary'>Read More</a></p>
                </article>
                <div class="post__bottom">
                    ...
                    <div class="post__socials">
                        <p class="post__social"><i class="mdi mdi-eye"></i> {{$pert->getViewsCount()}} <span>views</span></p>
