$(document).ready(function($) {
    ...
    if (typeof foo === 'string'){
        ...
        localStorage.clear();
        document.getElementById("refresh").href = "/admin/umkmpeta";
    }
    else {
        document.getElementById("refresh").href = "/admin/umkmpeta";
    ...
    function createMarkers() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.ajax({
                    dataType: "json",
                    url: window.location.origin + "/api/umkm-tahun-ini",
                    async:false,
...
// method that we will use to update the control based on feature properties passed
        ...
        info.update = function (props) {
            this._div.innerHTML = '<h4>' + 'Nilai INVESTASI UMKM Jateng ' + (new Date).getFullYear() + '</h4>'  +  (props ?
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            markerContent.innerHTML =
                ...
                ( ( loadedMarkersData[i]["jumlah_investasi"] >= 500000000) ? '<div class="ts-marker__feature">' + loadedMarkersData[i]["ribbon"] + '</div>' : "" ) +
                ( ( loadedMarkersData[i]["kab_kota_id"] !== undefined ) ? '<div class="ts-marker__title">' + loadedMarkersData[i]["kab_kota_id"] + '</div>' : "" ) +
                ( ( loadedMarkersData[i]["nib"] !== undefined) ? '<div class="ts-marker__info">' + loadedMarkersData[i]["nib"] + '</div>' : "" ) +
                ( ( loadedMarkersData[i]["marker_image"] !== undefined ) ? '<img style="max-width: 35px !important;" src=' + loadedMarkersData[i]["marker_image"] + '></img>' : '<img src="/public/map/icon/cjip.png"></img>' ) +
                '</a>' +
                '</div>';
    ...
    function createInfoBoxHTML(parameters) {
        ...
        infoboxHtml.innerHTML =
            ...
            ( ( loadedMarkersData[i]["jumlah_investasi"] >= 500000000 ) ? '<div class="ts-ribbon">' + loadedMarkersData[i]["ribbon"] + '</div>' : "" ) +
            ...
            ( ( loadedMarkersData[i]["kab_kota_id"] !== undefined) ? '<div class="ts-item__info-badge">' + loadedMarkersData[i]["kab_kota_id"] + '</div>' : "" ) +
            ( ( loadedMarkersData[i]["nib"] !== undefined && loadedMarkersData[i]["nib"].length > 0 ) ? '<h4>' + loadedMarkersData[i]["nib"] + '</h4>' : "" ) +
            '</figure>' +
            '</div>' +
            '</a>' +
            '</div>' +
